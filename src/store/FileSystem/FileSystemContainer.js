...
module.exports = class Contenedor {
  ...
  async save(obj) {
    try {
      ...
      throw new Error(`Error al escribir el OBJETO: ${JSON.stringify(obj)} en el ARCHIVO: ${this.rutaArchivo} Más info: ${error.message}`
      )
    }
  }
  ...
  async getById(id) {
    try {
      ...
      throw new Error(`Error al obtener el objeto con ID: ${id} del ARCHIVO: ${this.rutaArchivo}  Más info: ${error.message}` )
    }
  }
  ...
  async getAll() {
    try {
      if (!fs.existsSync(this.rutaArchivo))
        throw new Error(`No se puede leer el ARCHIVO: ${this.rutaArchivo} porque no existe`)
      ...
      throw new Error(`Error al leer el ARCHIVO: ${this.rutaArchivo}  Más info: ${error.message}`)
    }
  }
  ...
  async updateById(obj) {
    try {
      ...
      if (!fs.existsSync(this.rutaArchivo) || content.length < 1) {
        throw new Error(`No se puede editar el objeto con ID: ${obj.id} del ARCHIVO: ${this.rutaArchivo} porque no exist`)
      }
      ...
      if (!updateObj)
        throw new Error(`No se puede editar el objeto con ID: ${obj.id} del ARCHIVO: ${this.rutaArchivo} porque no existe`)

      return updateObj
    } catch (error) {
      throw new Error(`Error al editar el objeto con ID: ${obj.id} del ARCHIVO: ${this.rutaArchivo}  Más info: ${error.message}`)
    }
  }
  ...
  async deleteById(id) {
    try {
      ...
      throw new Error(`Error al eliminar el objeto con ID: ${id} del ARCHIVO: ${this.rutaArchivo} Más info: ${error.message}`)
    }
  }
  ...
  async deleteAll() {
    try {
      ...
      throw new Error(`Error al limpiar el ARCHIVO: ${this.rutaArchivo} Más info: ${error.message}`)
    }
  }
}